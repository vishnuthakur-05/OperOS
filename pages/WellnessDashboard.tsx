...
export const WellnessDashboard: React.FC<WellnessDashboardProps> = ({ user, tasks, onUpdateTask, onNavigate }) => {
   ...
   const handleSubmitLeave = (e: React.FormEvent) => {
      ...
      const newRequest = {
         type: 'Burnout', // Hardcoded as per requirements
         startDate: leaveStart,
         endDate: leaveEnd,
         reason: leaveReason,
         teamId: user.teamId || user.team, // Support both for robustness
      ...
      // Temporary Success Message (In real app, use a Toast component)
      alert("Success: Leave request sent. Redirecting to Leave Requests...");

      // Redirect to Leave Requests Page
      onNavigate('leaverequests');
                  ...
                  <div className="flex justify-between items-start">
                     <div>
                        <div className="flex items-center gap-3 mb-2">
                           <span className={`px-2 py-0.5 rounded text-xs font-bold text-white ${selectedTask.priority >= 5 ? 'bg-red-500' :
                              selectedTask.priority >= 3 ? 'bg-orange-400' : 'bg-blue-400'
                              }`}>
                              Priority {selectedTask.priority}
                           </span>
                           <span className={`px-2 py-0.5 rounded text-xs font-bold border ${selectedTask.status === 'DONE' ? 'bg-green-50 text-green-700 border-green-200' :
      ...
      <div className="space-y-8 animate-in fade-in duration-500">
         <div className="flex flex-col md:flex-row gap-6 items-stretch">
            ...
            <div className="w-full md:w-1/3 space-y-6">
               ...
               {/* Leave Management Widget (Moved & Tabbed) */}
